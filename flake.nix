  ...
  outputs = { self, nixpkgs, flake-utils, terranix }:
    ...
    // flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # ADR-015: provision the org-sync credential locally (a workflow/actor cannot).
        setSyncToken = pkgs.writeShellApplication {
          name = "set-sync-token";
          runtimeInputs = [ pkgs.python3 pkgs.gh ];
          text = ''exec python3 ${self}/claude/set_sync_token.py "$@"'';
        };

        appBootstrap = pkgs.writeShellApplication {
          name = "app-bootstrap";
          runtimeInputs = [ pkgs.python3 pkgs.gh ];
          text = ''exec python3 ${self}/claude/app_bootstrap.py "$@"'';
        };
      in
      {
        packages = {
          inherit syncAgentConfig orgCheck installHooks jiraCli setSyncToken appBootstrap;
          sync-agent-config = syncAgentConfig;
          org-check = orgCheck;
          install-hooks = installHooks;
          jira = jiraCli;
          set-sync-token = setSyncToken;
        ...
        apps = {
          ...
          set-sync-token = flake-utils.lib.mkApp { drv = setSyncToken; };
          app-bootstrap = flake-utils.lib.mkApp { drv = appBootstrap; };
