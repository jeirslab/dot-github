...
Repository flakes build on the org baseline (`github:{{ORG_GITHUB}}/.github`,
...
```bash
nix run github:{{ORG_GITHUB}}/.github#org-check                    # what is this repo missing?
nix run github:{{ORG_GITHUB}}/.github#sync-agent-config -- check --target .
nix run github:{{ORG_GITHUB}}/.github#sync-agent-config -- apply --target . --version manual
```
