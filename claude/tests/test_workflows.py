...
class WorkflowPolicyTests(unittest.TestCase):
    ...
    def test_targets_confinement(self):
        ...
        r = targets.resolve("acme", ROOT / "claude" / "repos.txt", "acme/app, acme/other", {"ORG_FEAT_TEST": "false"})
        self.assertEqual(r["repos"], ["acme/app", "acme/other"]); self.assertEqual(r["ref"], "")
        r = targets.resolve("acme", ROOT / "claude" / "repos.txt", "", {})
        self.assertTrue(r["repos"]); self.assertTrue(all("/" in x for x in r["repos"]))
