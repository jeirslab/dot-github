...
| [ADR-015](adr-015-github-app-automation-identity.md) | A GitHub App is the org automation identity (workflows mint per-run tokens); the root credential is provisioned locally (`set-sync-token` / `app-bootstrap`), never by a workflow or the actor | Accepted |
