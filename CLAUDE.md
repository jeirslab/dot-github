<!-- BEGIN ORG BASELINE -->
<!-- Managed by the organization's .github repository (claude/CLAUDE.md, ADR-001). Version: v1.0.7-18-gce38e80
...
# This repository (the org `.github`)

Organization-level home for reusable workflows, the org agent configuration (`claude/`),
the org baseline dev-environment flake, and organization ADRs (`docs/adr/`).

- Jira project for work here: `INFRA`.
- Checks: `python3 -m unittest discover -s claude/tests`, `nix flake check`,
  `python3 .github/scripts/generate-workflow-docs.py` after touching a public workflow
  (`WORKFLOWS.md` is generated; never hand-edit it).
- Workflows are thin shells: logic lives in `claude/*.py`, `claude/*.sh`, or Nix, and
  must run locally with the same inputs (ADR-002 R12).
- `claude/CLAUDE.md` is the template that ships to every repository; edit it there, not
  in the block above, which the sync overwrites.
- Significant changes to workflows or `claude/` get an ADR in `docs/adr/`.
